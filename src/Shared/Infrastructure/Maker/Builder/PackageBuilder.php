...
final class PackageBuilder
{
    public function addMappingPathToApiPlatform(
        ...
        $node = new Node\Scalar\String_(
            sprintf('%s/src/%s/Shared/Infrastructure/Persistence/Doctrine/ORM/Entity', '%kernel.project_dir%', $configuration->getPackagePath()),
        );
    ...
    public function addMappingToDoctrine(
        ...
        $dir = new Node\Expr\ArrayItem(
            new Node\Scalar\String_(sprintf('%s/src/%s/Shared/Infrastructure/Persistence/Doctrine/ORM/Entity', '%kernel.project_dir%', $configuration->getPackagePath())),
